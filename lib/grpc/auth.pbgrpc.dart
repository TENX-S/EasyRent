///
//  Generated code. Do not modify.
//  source: auth.proto
//
// @dart = 2.12
// ignore_for_file: annotate_overrides,camel_case_types,unnecessary_const,non_constant_identifier_names,library_prefixes,unused_import,unused_shown_name,return_of_invalid_type,unnecessary_this,prefer_final_fields

import 'dart:async' as $async;

import 'dart:core' as $core;

import 'package:grpc/service_api.dart' as $grpc;
import 'auth.pb.dart' as $3;
export 'auth.pb.dart';

class AuthenticateClient extends $grpc.Client {
  static final _$onLogin = $grpc.ClientMethod<$3.AuthRequest, $3.LoginReply>(
      '/easyrent.auth.Authenticate/OnLogin',
      ($3.AuthRequest value) => value.writeToBuffer(),
      ($core.List<$core.int> value) => $3.LoginReply.fromBuffer(value));
  static final _$onRegister =
      $grpc.ClientMethod<$3.AuthRequest, $3.RegisterReply>(
          '/easyrent.auth.Authenticate/OnRegister',
          ($3.AuthRequest value) => value.writeToBuffer(),
          ($core.List<$core.int> value) => $3.RegisterReply.fromBuffer(value));

  AuthenticateClient($grpc.ClientChannel channel,
      {$grpc.CallOptions? options,
      $core.Iterable<$grpc.ClientInterceptor>? interceptors})
      : super(channel, options: options, interceptors: interceptors);

  $grpc.ResponseFuture<$3.LoginReply> onLogin($3.AuthRequest request,
      {$grpc.CallOptions? options}) {
    return $createUnaryCall(_$onLogin, request, options: options);
  }

  $grpc.ResponseFuture<$3.RegisterReply> onRegister($3.AuthRequest request,
      {$grpc.CallOptions? options}) {
    return $createUnaryCall(_$onRegister, request, options: options);
  }
}

abstract class AuthenticateServiceBase extends $grpc.Service {
  $core.String get $name => 'easyrent.auth.Authenticate';

  AuthenticateServiceBase() {
    $addMethod($grpc.ServiceMethod<$3.AuthRequest, $3.LoginReply>(
        'OnLogin',
        onLogin_Pre,
        false,
        false,
        ($core.List<$core.int> value) => $3.AuthRequest.fromBuffer(value),
        ($3.LoginReply value) => value.writeToBuffer()));
    $addMethod($grpc.ServiceMethod<$3.AuthRequest, $3.RegisterReply>(
        'OnRegister',
        onRegister_Pre,
        false,
        false,
        ($core.List<$core.int> value) => $3.AuthRequest.fromBuffer(value),
        ($3.RegisterReply value) => value.writeToBuffer()));
  }

  $async.Future<$3.LoginReply> onLogin_Pre(
      $grpc.ServiceCall call, $async.Future<$3.AuthRequest> request) async {
    return onLogin(call, await request);
  }

  $async.Future<$3.RegisterReply> onRegister_Pre(
      $grpc.ServiceCall call, $async.Future<$3.AuthRequest> request) async {
    return onRegister(call, await request);
  }

  $async.Future<$3.LoginReply> onLogin(
      $grpc.ServiceCall call, $3.AuthRequest request);
  $async.Future<$3.RegisterReply> onRegister(
      $grpc.ServiceCall call, $3.AuthRequest request);
}
